...
# Historial de versiones
  ...
  **Versión**   **Fecha**      **Autor**    **Descripción**
  ------------- -------------- ------------ ---------------------------------------
  0.1.0         2026-05-13     \<AUTOR\>    Versión inicial (borrador)

  0.1.1         2026-05-20     \<AUTOR\>    Correcciones: snippet ApiResponseWrapper completo (constructor y getters); imports agregados en OpenApiConfig; aclaración traceId/trace.id en A.2; índice logs unificado a onp-logs-* en sección 2.4 y A.3.
...
En ONP, la instrumentación produce trazas distribuidas: registros detallados del recorrido de cada petición HTTP a través del servicio, incluyendo las consultas a Oracle, las llamadas a servicios externos (RENIEC, SUNAT, etc.) y las operaciones de negocio relevantes.
  ...
  ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  **IMPORTANTE:** Seguir esta guía es obligatorio para todos los servicios web que se desplieguen en los entornos DEV, QA y PROD. Un servicio no instrumentado es invisible para el equipo de operaciones y dificulta el diagnóstico de problemas en producción.
  ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
### 1.3.2 application.properties — configuración base

**Dónde:** Solo en `src/main/resources/application.properties` (el archivo base sin sufijo de perfil). Spring Boot lo carga siempre, sin importar el entorno activo. No repetir esta configuración en los archivos de cada entorno.
...
# ═══════════════════════════════════════════════════════════════
spring.application.name=onp-<sistema>-<modulo>
# Versión del servicio
management.info.env.enabled=true
info.app.version=@project.version@
# Protocolo de exportación: HTTP (puerto 4318)
management.otlp.tracing.export.protocol=http/protobuf
...
### 1.3.3 DEV --- application-dev.properties
...
**Dónde:** `src/main/resources/application-dev.properties`

```properties
management.otlp.tracing.endpoint=http://otel-collector.otel-dev.svc.cluster.local:4318/v1/traces
management.otlp.logging.endpoint=http://otel-collector.otel-dev.svc.cluster.local:4318/v1/logs
management.tracing.sampling.probability=1.0
otel.resource.attributes=deployment.environment=development
app.environment=development
```

### 1.3.4 QA --- application-qa.properties

**Obligatorio.** Mismo criterio que DEV — sin este archivo el servicio no emite telemetría en QA.

**Dónde:** `src/main/resources/application-qa.properties`

```properties
management.otlp.tracing.endpoint=http://otel-collector.otel-qa.svc.cluster.local:4318/v1/traces
management.otlp.logging.endpoint=http://otel-collector.otel-qa.svc.cluster.local:4318/v1/logs
management.tracing.sampling.probability=1.0
otel.resource.attributes=deployment.environment=quality
app.environment=quality
```

### 1.3.5 PROD --- application-prod.properties
...
**Dónde:** `src/main/resources/application-prod.properties`

```properties
management.otlp.tracing.endpoint=http://otel-collector.otel.svc.cluster.local:4318/v1/traces
management.otlp.logging.endpoint=http://otel-collector.otel.svc.cluster.local:4318/v1/logs
...
**Requiere:** `spring-boot-starter-security` en el `pom.xml` del proyecto. Si Spring Security no está presente, el campo `user.id` siempre registrará `"anonymous"`. El `RequestIdFilter` (sección 3.3.2) debe estar presente para que el campo `http.request.id` sea accesible en el log canónico.
...
> **Nota sobre `@Order(1)`:** El filtro se registra con orden 1, posterior al `FilterChainProxy` de Spring Security (orden -100). Esto garantiza que el filtro corre dentro del contexto de seguridad — `SecurityContextHolder` está disponible y el usuario ya está autenticado cuando se captura `user.id`.
...
La documentación de APIs se basa en la especificación OpenAPI 3.0, el estándar de la industria para describir APIs REST de forma legible tanto para humanos como para herramientas automatizadas. En ONP, la documentación se genera automáticamente mediante anotaciones en el código usando SpringDoc, y es visualizable a través del Swagger UI.
  ...
  -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  **IMPORTANTE:** La documentación Swagger es un requisito de entrega, no una mejora opcional. Un servicio sin documentación Swagger no se considera completo y no debe pasar a revisión de código.
  -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
```json
{
  ...
  "data": null,
  "errors": [
    { "field": "regimen", "message": "El campo es obligatorio." },
    { "field": "fechaInicioLaboral", "message": "El formato debe ser ISO 8601." }
  ],
...
## A.4 Tabla de codDetRespuesta --- resumen rápido
  ...
  **Código**   **Categoría**   **Descripción**                      **codHttp**
  ...
  ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  **NOTA:** Para la tabla completa de codDetRespuesta consultar la sección 3.3.3. Para la especificación completa de atributos OTEL: https://opentelemetry.io/docs/specs/semconv/
  ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
